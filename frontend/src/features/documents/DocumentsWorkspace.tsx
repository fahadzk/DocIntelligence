...
export function DocumentsWorkspace({ project }: { project: Project }) {
  ...
  return <section className="documents-workspace">
    <div className="documents-heading"><div><p className="eyebrow">Project / Documents</p><h2>Documents</h2><p>Source material stored with this project.</p></div><Button variant="primary" onClick={() => picker.current?.click()} disabled={uploading}>{uploading ? "Adding…" : "+ Add documents"}</Button></div>
