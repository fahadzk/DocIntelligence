...
class LabSearchRepository:
    ...
    def status(self, project_id: str) -> list[dict]:
        with self.connect() as connection:
            rows = connection.execute("""
                ...
                FROM documents d LEFT JOIN lab_document_indexes i ON i.document_id=d.id
                WHERE d.project_id=? ORDER BY d.created_at DESC
            """, (project_id,)).fetchall()
        return [dict(row) for row in rows]

    def rebuild_fts(self) -> None:
        """FTS is derived entirely from lab_passages and can be recreated safely."""
        with self.connect() as connection:
            connection.execute("DROP TABLE IF EXISTS lab_passages_fts")
            connection.execute("CREATE VIRTUAL TABLE lab_passages_fts USING fts5(id UNINDEXED, project_id UNINDEXED, text, tokenize='unicode61')")
            connection.execute("INSERT INTO lab_passages_fts (id, project_id, text) SELECT id, project_id, text FROM lab_passages")

    def mark(self, project_id: str, document_id: str, status: str, stage: str,
             version: str, content_hash: str, error: str | None = None) -> None:
        with self.connect() as connection:
            connection.execute("""
                INSERT INTO lab_document_indexes VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ...
    def replace(self, project_id: str, document_id: str, lab_passages: list[dict],
                version: str, content_hash: str) -> None:
        with self.connect() as connection:
            ids = [row[0] for row in connection.execute(
                "SELECT id FROM lab_passages WHERE project_id=? AND document_id=?",
                (project_id, document_id))]
            connection.executemany("DELETE FROM lab_passages_fts WHERE id=?", [(item,) for item in ids])
            connection.execute("DELETE FROM lab_passages WHERE project_id=? AND document_id=?", (project_id, document_id))
            connection.executemany("""
                INSERT INTO lab_passages VALUES (:id, :project_id, :document_id, :segment_index,
                    :chunk_index, :label, :page_number, :paragraph_number,
                    :start_offset, :end_offset, :text, :content_hash, :index_version)
            """, lab_passages)
            connection.executemany(
                "INSERT INTO lab_passages_fts (id, project_id, text) VALUES (?, ?, ?)",
                [(item["id"], project_id, item["text"]) for item in lab_passages])
            connection.execute("""
                INSERT INTO lab_document_indexes VALUES (?, ?, 'ready', 'complete', ?, ?, NULL, ?)
    ...
    def delete(self, project_id: str, document_id: str) -> None:
        with self.connect() as connection:
            ...
            connection.executemany("DELETE FROM lab_passages_fts WHERE id=?", [(item,) for item in ids])
            connection.execute("DELETE FROM lab_passages WHERE project_id=? AND document_id=?", (project_id, document_id))
            connection.execute("DELETE FROM lab_document_indexes WHERE project_id=? AND document_id=?", (project_id, document_id))

    def get(self, project_id: str, passage_id: str) -> dict | None:
        with self.connect() as connection:
            row = connection.execute("""
                SELECT p.*, d.display_name, d.file_type FROM lab_passages p
    ...
    def keyword(self, project_id: str, query: str, limit: int = 30) -> list[str]:
        ...
        with self.connect() as connection:
            rows = connection.execute("""
                SELECT f.id FROM lab_passages_fts f
                JOIN lab_passages p ON p.id=f.id AND p.project_id=f.project_id
                ...
                ORDER BY bm25(lab_passages_fts) LIMIT ?
    ...
    def valid_ids(self, project_id: str) -> set[str]:
        with self.connect() as connection:
            rows = connection.execute("""
                SELECT p.id FROM lab_passages p JOIN documents d ON d.id=p.document_id
                JOIN lab_document_indexes i ON i.document_id=d.id
