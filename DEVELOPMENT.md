...
The **Activity** tab calls `GET /api/projects/{id}/activity` only while it is open. Its default refresh is 5 seconds; users may select 10, 15, 30, or 60 seconds, disable polling, or refresh manually. The log deliberately stores operational metadata and durations only, rather than document text or user prompts.
